...
sudo nala install -y build-essential vim
...
if [ $INSTALL_C == true ]; then
  display "C Start"
  sudo nala install -y libcriterion-dev cppcheck gdb valgrind lldb gcovr libncurses-dev libcsfml-dev
  #"$SCRIPT_DIR/criterion/install_criterion.sh"
  log "C End"
fi

display "Start Framwork & Header Updates"
sudo nala install -y linux-headers-"$(uname -r)" software-properties-common laptop-mode-tools
...
cargo install fcp
sudo nala install -y eza safe-rm tldr bat ripgrep fzf fd-find
...
if [ ! "$(command -v thunar)" ]; then
  sudo nala install -y thunar thunar-archive-plugin thunar-media-tags-plugin tumbler
